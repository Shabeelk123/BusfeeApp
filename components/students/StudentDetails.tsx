...
// ─── Theme (Stitch: "Academic Transit Logistics" — matches the Admin/Class Dashboards) ─
const T = {
  background: "#f7fafc",
  navy: "#1a2b48",
  navyLight: "#e8ebf2",
...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={{ color: toneColor, fontSize: 10, fontWeight: "700" }}>
          {label}
        </Text>
      </View>
      <Text
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: 11, fontWeight: "800", color: tone.color }}>
        {status}
      </Text>
    </View>
  );
    ...
    >
      ...
      {expanded && (
        ...
        >
          ...
          {canCollectPayment && pending > 0 && entry.status !== "Excluded" && (
            <AppButton
...
export default function StudentDetailsScreen({ role, baseRoute }: Props) {
  ...
  // How many months this student still owes fee for — more useful to a
  // CLASS account than its own grade/division.
  const pendingMonthsCount = useMemo(
    () => ledger.filter((m) => !m.excluded && m.fee - m.paid_amount > 0).length,
    [ledger],
  );
      ...
      >
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            >
              <Ionicons name="school-outline" size={28} color={T.navy} />
            </View>
            <View style={{ flex: 1 }}>
              <Text
                style={{
          ...
          {/* CLASS already knows its own grade/division — show info a
                        class teacher can't already see instead. ADMIN/COORDINATOR
                        manage multiple classes, so they still need Grade/Division. */}
            ...
            {role === "CLASS" ? (
              <>
                ...
                >
                  ...
                  >
                    {pendingMonthsCount === 0 ? "None" : pendingMonthsCount}
                  </Text>
                </View>
              </>
            ) : (
        ...
        <View style={{ flexDirection: "row", gap: 10, marginBottom: 16 }}>
          <SummaryCard label="Monthly Fee" value={`₹${student.monthly_fee}`} />
